...
int GetHTTPHeaders(Connection *con);
...
void CrawlHosts(Connection *con)
{
	...
	if (GetHTTPContent(con, MAX_READ_SIZE))
	{
...
int GetHTTPHeaders(Connection *con)
{
	if (con->raw_host != NULL &&
		SendGetRequest(con, REQ_GET_HEADER_CLOSE, con->raw_host, con->port_numeric) >= 0 &&
		Read(con->sck, con->http_data.received_data, MAX_READ_SIZE) >= 0)
	{

		// handle This (Check for Bytes in body)
		// strstr(con->http_data.received_data, "\"HTTP/1.1 200 OK") == NULL
		return 1;
	}
	...
	while ((*n_remote < MAX_LINKS) && ((html = strstr(html, "href=")) != NULL))
	{
		...
		if (encase == '\'' || encase == '"')
		{
			...
			if (end_of_link != NULL && size > 0 && size < MAX_LINK_LEN)
			{
				...
				if (GetNewLocation(temp) == NULL)
				{ /* temp is a local link */
